...
RdmaCM::RdmaCM()
{
    ...
    HENSURE_ERRNO(m_channel != nullptr);
    HENSURE_ERRNO(rdma_create_id(m_channel, &m_id, nullptr, RDMA_PS_TCP) == 0);
}
...
{
    {
        ...
        HENSURE_ERRNO(inet_aton(addr.c_str(), reinterpret_cast<in_addr*>(&sock_addr.sin_addr.s_addr)) >= 0);
        HENSURE_ERRNO(rdma_resolve_addr(m_connection_id, nullptr, reinterpret_cast<sockaddr*>(&sock_addr), timeout_ms) == 0);
    }
    ...
    while(!done)
    {
        ...
        switch(event.event)
        {
            case RDMA_CM_EVENT_CONNECT_REQUEST:
            {
                ...
                memset_zero(&param);
                HENSURE_ERRNO(rdma_accept(event->id, &param) == 0);
            }
                break;
